...
## 2026-08-02｜参考图全界面 UI 重构

- 建立共享蓝白视觉令牌、页面背景、3D 格式文件插画和 ArkUI 展示组件，全部资源随包离线。
- 按用户八屏参考图重构首页、图片、文档、PDF、档案与时间、单位和图片工具，并以同一系统重构音频、视频和关于页。
- 独立页面统一 `NavigationMode.Stack`、响应式内容宽度和明确的空、禁用、错误、完成状态；phone、tablet、2in1 使用断点调整布局。
- 未开发格式保留相同布局，以灰色禁用并显示“功能正在开发”；可用格式仍绑定原有真实转换引擎。
- 关于页删除占位邮箱和网址，只保留可验证的单机离线、隐私与版本说明。
- 参考原图拆分、运行截图、交互结果和设备限制见 `docs/qa/2026-08-02-reference-ui-regression.md`。
- 当前边界：2in1 收尾时 HDC 离线，tablet 无独立设备；发布签名、主体/法务元数据与未集成离线引擎仍需外部条件。

## 2026-08-03｜主工作区 UI 版本错位修复

- 根因不是 UI 源码回退，而是 DevEco 打开的项目根目录仍停在旧提交 `d80645a`；昨天的参考 UI 位于 `3c67edf`，两套工作树构建了同一包名，旧包覆盖后表现为界面回退。
- 将主目录安全切换到跟踪 `origin/master` 的本地 `master`，根目录、远端和参考 UI 分支统一到 `3c67edf`。
- 从主目录重新执行全量契约、类型检查、标准门禁和 HarmonyOS 构建，并覆盖安装到 phone `5555` 与 2in1 `5557`。
- 两台设备均显示新版蓝白首页、主插画和“多种格式 · 快速转换 · 安全可靠”文案。
- 保留用户已有 `Crash_365312.dmp`，新增根目录 `.dmp` 忽略规则，避免临时崩溃转储进入提交。
- 证据：`docs/qa/2026-08-03-workspace-version-fix.md`。

## 2026-08-03｜首页与关于页参考图精确还原

- 首页按新参考图更新 32vp 品牌标题、蓝色副标题、格式文件主插画、八张双列入口卡片及双标签底栏。
- 关于页更新为带环形箭头的应用图标、四枚独立隐私图标、四行本地隐私声明和版本信息。
- 功能卡图标增加圆形浅蓝承托，标题与说明字号重新收敛；phone 主插画高度经设备截图二次调整，八张卡片均完整露出。
...
- PDF 反向转换不再把 PDF 二进制先按 UTF-8 解码；提取器直接接收实际读取的 `ArrayBuffer`，并以包含非 UTF-8 字节的样本完成回归。
- 文档批量转换改为先写沙箱临时结果，再由一次系统 Picker 请求返回全部目标 URI；取消、数量不匹配和复制失败均有明确状态，临时文件统一清理。
- 视频普通转码、MOV 重封装和 MP4 音轨提取统一从已打开 fd 获取源文件大小，兼容 Picker URI。
- 音频同格式文件跳过时推进进度并显示跳过数量。
