...
- 状态：`blocked`（UI 实现与自动验证完成；最终 2in1 全页面回归因设备端点离线未完成）
...
- [x] 确认未开发能力采用同布局灰色禁用态。
- [x] 完成 UI 重构设计并获得方案确认。
- [x] 建立共享视觉令牌、3D 资源和 ArkUI 组件。
- [x] 按参考图重构八个已设计页面。
- [x] 按相似风格重构音频、视频、关于及全部状态页。
- [ ] 完成 phone、2in1 截图对照和交互回归（phone 已完成；2in1 仅有首页、图片、文档、视频的本分支证据，最终包全页面覆盖因 `127.0.0.1:5557` 离线而阻塞）。
- [x] 通过全量测试、类型检查、静态门禁和 HarmonyOS 构建。
- [x] 回填设计、开发文档、变更记录和 QA 证据。
...
### T-20260803-002 首页与关于页参考图精确还原

- 状态：`done`
- 目标：首页按用户图二还原，关于页按用户图一还原；不改变业务入口、路由、离线能力和真实功能矩阵。
- 范围：`Index.ets`、`AboutPage.ets`、`FunctionTile.ets`、必要媒体资源、UI 契约和设备截图。

检查项：

- [x] 固化两张用户参考图与布局尺寸要求。
- [x] 先新增首页/关于页结构契约并验证 RED。
- [x] 实现 phone 参考布局和宽屏响应式布局。
- [x] 通过全量契约、类型检查、标准门禁和 HarmonyOS 构建。
- [x] 在 phone 与 2in1 安装验证并回填 QA 证据。

完成证据：`docs/qa/2026-08-03-home-about-reference-ui.md`。

### T-20260803-003 应用图标上架规范替换

- 状态：`done`
- 目标：使用用户提供的新图标统一替换桌面、启动窗口和关于页图标，并完成构建与设备验证。
- 范围：应用图标媒体资源、资源一致性契约、phone/2in1 截图及相关文档。

检查项：

- [x] 固化图标替换设计与上架边界。
- [x] 新增图标资源一致性契约并验证 RED。
- [x] 生成并替换三处标准 PNG 图标资源。
- [x] 通过全量契约、类型检查、静态门禁和 HarmonyOS 构建。
- [x] 在 phone 与 2in1 安装验证并回填 QA 证据。
