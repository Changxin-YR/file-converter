...
### T-20260804-001 AppGallery 审核功能下架与字号整改

- 状态：`blocked`
- 目标：移除审核实测失败或仍显示“开发中”的图片、音频、视频和档案转换能力，消除小于 10fp 的用户可见文字，确保重新上架时只呈现已验证的离线功能。
- 范围：图片格式、视频目标格式、音频首页入口/页面/服务、档案格式、路由、相应自动化契约，以及产品设计、开发说明、变更和 QA 记录。
- 非范围：不修改已验证的文档转换、图片基础转换、视频 MP4/M4A、ZIP、时区换算、包名、签名、版本或网络/权限配置。

检查项：

- [x] 下架未验证的格式选项和音频转换入口。
- [x] 移除与下架能力对应的路由及不再使用的实现。
- [x] 所有用户可见文本字号不低于 10fp。
- [x] 更新设计、开发、变更和 QA 记录。
- [ ] 通过 HarmonyOS 签名构建和设备复验；受既有签名配置引用的本机缺失证书阻塞。
...
### T-20260803-005 第三轮资源与 ArkTS 审查建议核实修复

- 状态：`done`
- 目标：核实第三轮审查报告，修复确有根因且在当前能力边界内的问题；将已修复、误报或需外部条件的项如实记录。
- 范围：主题颜色资源、通用组件状态、PDF 字符编码及相关自动契约、设计/开发/QA 记录。

检查项：

- [x] 对比浅色/深色资源的完整定义并建立回归契约。
- [x] 验证并修复禁用选项的选中态冲突。
- [x] 核实 PDF 非 BMP 字符和现有视频/PDF/批量保存建议的当前状态。
- [x] 完成静态门禁、HarmonyOS 构建及与风险相称的验证记录。

完成证据：`docs/qa/2026-08-03-third-round-review-remediation.md`。深色视觉截图、非 MP4→M4A 的系统 Picker 闭环和 CJK PDF 目标阅读器复验保留为明确的设备/资产边界。

### T-20260803-006 应用名称变更为文件格式盒

- 状态：`done`
- 目标：将用户可见应用名改为“文件格式盒”，保持应用身份与已验证图标资源不变。
- 范围：`app_name` 资源、可见产品文案、回归契约、设计/开发/QA 记录和 phone 验证。

检查项：

- [x] 更新所有用户可见的 `app_name` 资源与名称契约。
- [x] 不修改包名、应用 ID、签名、版本、路由、权限或图标内容。
- [x] 通过标准检查和 HarmonyOS 构建，并安装到 phone 核验名称。

完成证据：`docs/qa/2026-08-03-app-name-change.md`。
